    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-2 mb-2">
                <div class="card ribbon-box">
                    <select name="subject_id" id="subject_id" class="form-control" onchange="filter_test()">
                        ...
                        ?>
                            <option value="<?php echo $subjects[$i]->id ?>" <?php if ($subjects[$i]->id == $subject_id) {
                                                                                echo 'selected';
                                                                            } else {
                                                                                '';
                                                                            } ?>><?php echo $subjects[$i]->name ?></option>
                        <?php } ?>
                    </select>
                </div>
            </div>
            <div class="col-lg-2 mb-2">
                <div class="card ribbon-box">
                    <select name="grade_id" id="grade_id" class="form-control" onchange="filter_test()">
                        ...
                        ?>
                            <option value="<?php echo $grades[$i]->id ?>" <?php if ($grades[$i]->id == $grade_id) {
                                                                                echo 'selected';
                                                                            } else {
                                                                                '';
                                                                            } ?>><?php echo $grades[$i]->name ?></option>
                        <?php } ?>
                    </select>
                </div>
            </div>
            <div class="col-lg-2 mb-2">
                <div class="card ribbon-box">
                    <select name="status_id" id="status_id" class="form-control" onchange="filter_test()">
                        ...
                        ?>
                            <option value="<?php echo $statuses[$i]->id ?>" <?php if ($statuses[$i]->id == $status_id) {
                                                                                echo 'selected';
                                                                            } else {
                                                                                '';
                                                                            } ?>><?php echo $statuses[$i]->name ?></option>
                        <?php } ?>
                    </select>
                </div>
            </div>
            <div class="col-lg-1 mb-2">
                <div class="card ribbon-box">
                    <button class="btn btn-primary" id="resetFilter">Đặt lại</button>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            ...
            <?php } else { ?>
                <div class="col-lg-12 mb-1">
                    <span class="btn btn-warning text-white title form-control mb-3">Chưa có đề cho khối và môn đã chọn!</span>
                </div>
            <?php } ?>
            <?php
            ...
            <div class="col-lg-12 mb-1">
                <nav>
                    <ul class="pagination pagination-rounded mb-0 justify-center">
                        <?php
                        if ($current_page > 1) {
                            echo '<li class="page-item"><a class="page-link" href="index.php?action=list_test_filter&subject_id=' . $_GET['subject_id'] . '&grade_id=' . $_GET['grade_id'] . '&status_id=' . $_GET['status_id'] . '&page=' . ($current_page - 1) . '" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>';
                        }

                        for ($i = 1; $i <= $number_of_pages; $i++) {
                            echo '<li class="page-item"><a class="page-link" href="index.php?action=list_test_filter&subject_id=' . $_GET['subject_id'] . '&grade_id=' . $_GET['grade_id'] . '&status_id=' . $_GET['status_id'] . '&page=' . $i . '">' . $i . '</a></li>';
                        }

                        if ($i > $current_page) {
                            echo '<li class="page-item"><a class="page-link" href="index.php?action=list_test_filter&subject_id=' . $_GET['subject_id'] . '&grade_id=' . $_GET['grade_id'] . '&status_id=' . $_GET['status_id'] . '&page=' . ($current_page + 1) . '" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>';
                        }
                        ?>
                    </ul>
                </nav>
